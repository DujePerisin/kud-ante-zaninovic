...
//                 ? { field: link, target: "_blank", rel: "noopener noreferrer" }
//                 : {};
...
import { FC, useMemo, useState, type CSSProperties } from "react";
import type { Content } from "@prismicio/client";
import type { SliceComponentProps } from "@prismicio/react";
import { PrismicRichText } from "@prismicio/react";
import { PrismicNextImage, PrismicNextLink } from "@prismicio/next";
            ...
            <div className="mt-3 flex gap-4 justify-center lg:justify-start ml-2.5">
              {items.map((_, i) => (
                <button
                  ...
                  aria-label={`Prikaži karticu ${i + 1}`}
                  className={[
                    "h-9 w-9 rounded-full transition-all md:h-11 md:w-11 flex items-center justify-center text-xs font-semibold",
                    active === i
                      ? "bg-green-600 ring-4 ring-violet-600/20 text-white"
                      : "bg-gray-300/70 hover:bg-gray-200 text-gray-800",
                  ].join(" ")}
                >
                  <span>{i + 1}</span>
                </button>
        ...
        <div className="relative h-[280px] sm:h-[300px] md:h-[320px]">
          <div
            className={[
              "absolute top-0 [transform-style:preserve-3d] [perspective:800px]",
              // positioning
              "left-1/2 -translate-x-1/2",         // centered on < md
              "md:left-1/2 md:-translate-x-1/2",   // centered on md
              "lg:left-[60px] lg:translate-x-0",   // shifted left on lg+
              // stage dimensions
              "h-[240px] w-[320px]",              // <640px
              "sm:h-[280px] sm:w-[340px]",        // ≥640px
              "md:h-[289px] md:w-[400px]",        // ≥768px
              // small scale on xs
              "scale-[0.9] sm:scale-100",
              // card sizes via CSS vars
              "[--card-w:264px] [--card-h:165px]",   // <640px
              "sm:[--card-w:350px] sm:[--card-h:220px]",
              "md:[--card-w:410px] md:[--card-h:270px]",
              "lg:[--card-w:340px] lg:[--card-h:215px]",   // lg
              "xl:[--card-w:390px] xl:[--card-h:247px]",   // ≥1280px
            ].join(" ")}
          >
            {ordered.map(({ idx, preset }) => {
              const item = items[idx];
                ...
                <>
                  {image?.url ? (
                    ...
                    <div
                      ...
                      style={{
                        background:
                          idx === 0 ? "#1d4ed8" : idx === 1 ? "#22c55e" : "#fecdd3",
                      }}
                    >
                      Slika
                    </div>
